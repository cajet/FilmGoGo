...
public class AddShowMovie extends Activity{
    ...
    private void init() {
       ...
       position = intent.getIntExtra("position", -1);
        ...
        movie_name.setText("电影名： "+app.getOldMovieList().get(position).get("name").toString());
    }
    ...
    private void setListener() {
        commit_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Toast.makeText(AddShowMovie.this, "上映电影成功", Toast.LENGTH_SHORT).show();
                finish();
            }
        });
    ...
    Runnable networkTask = new Runnable() {
        ...
        public void run() {
            ...
            try {
                ...
                param.put("name",app.getOldMovieList().get(position).get("name"));
                param.put("description", app.getOldMovieList().get(position).get("description"));
                param.put("image", app.getOldMovieList().get(position).get("icon_url"));
                param.put("time", time.getText().toString());
                param.put("price", movie_price.getText().toString());
                param.put("type", app.getOldMovieList().get(position).get("type"));
                param.put("star", app.getOldMovieList().get(position).get("star"));
                param.put("score", app.getOldMovieList().get(position).get("score"));
                // 绑定到请求 Entry
